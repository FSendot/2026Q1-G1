...
There is **no public ingress** to the VPC. The VGW is attached and route propagation is enabled, but no `aws_customer_gateway` or `aws_vpn_connection` is provisioned in this iteration.
...
- **No on-prem simulation VPC, CGW or `aws_vpn_connection`** — deferred behind a future `enable_onprem_sim` flag. Only the VGW is created today.
...
| ≥1 custom module                      | `modules/network`, `modules/queue`, `modules/data_store`, `modules/compute` (4).                                |
| ≥4 Terraform functions                | `merge`, `format`, `cidrsubnet`, `toset`, `replace`, `length`, `can`, `cidrhost`, `jsonencode`, `contains`, `slice`. |
| ≥3 meta-arguments                     | `for_each` (gateway and interface endpoints), `lifecycle { ignore_changes }` (ECS service desired_count), `depends_on` (service → SG egress rule), plus `validation` blocks on every variable. |
